...
サイト、ページ、URL、ブラウザ、または単にウィンドウと指定した場合は、JARVIS内のブラウザを標準で使います。外部ブラウザはアプリ名が明示された場合だけ対象にします。各ウィンドウ上部のURL欄には、完全なURLまたはホスト名を直接入力して移動できます。
...
AIは最小化中も含む一覧を確認して対象を選びます。対象が曖昧な場合は、タイトルや位置などを確認します。ページのクリック・入力には従来の確認処理が適用されます。移動・サイズ変更・最小化はJARVIS内のブラウザが対象です。Chromeから取り込んだCookieは、同じ専用データストアを使うすべてのアプリ内ブラウザウィンドウで利用できます。すでに表示中のページには再読み込み後に反映されます。
...
外部アプリ操作に必要なmacOSのアクセシビリティが未許可の場合、初回起動時にmacOS標準の案内を一度表示します。あとから許可する場合は、JARVISの設定の「外部アプリとウィンドウ」に未許可時だけ表示される「アクセシビリティを許可する」ボタンを使ってください。許可状態は設定へ戻ったときに再確認します。反映されない場合はJARVISを再起動してください。この許可はJARVIS内のブラウザ操作には不要です。
...
CI check enforces this against the pull request's base commit. Branch protection requires
the aggregate `CI passed` check with strict base-branch synchronization, so a pull request
must rerun the version check whenever `main` advances before it can merge.
